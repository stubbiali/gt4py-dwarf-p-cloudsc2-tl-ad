...
from cloudsc2_gt4py.physics.tangent_linear.validation import TaylorTest
...
from ifs_physics_common.timing import Timer
...
def core(config: PythonConfig, io_config: IOConfig) -> PythonConfig:
    ...
    tt = TaylorTest(
        computational_grid,
        factor1=0.01,
        factor2s=tuple(float(10 ** -(i + 1)) for i in range(0, 10)),
        kflag=1,
        lphylin=True,
        ldrain1d=False,
